...
import (
	...
	"microservice/simpleServerDemo2/Server"
	"microservice/simpleServerDemo2/Tool"
	"microservice/simpleServerDemo2/Transport"
	"net/http"
	"os"
	"os/signal"
	"syscall"
...
// 在simpleServerDemo基础上新增服务注册与发现和其他组件的扩充
// 使用 https://github.com/gorilla/mux 作为路由（也可以使用其他，例如gin、goframe）
...
func main() {
	...
	// 加入中间件
	Bye := Endpoint.MiddleWare(Endpoint.B)(Endpoint.MakeServerEndPointBye(s))
	...
	go func() {
		err := Tool.RegService("127.0.0.1:8500", "2", "测试", "127.0.0.1", 8004, "5s", "http://172.19.29.250:8004/health", "test")
		if err != nil {
			errChan <- err
		}
		_ = http.ListenAndServe("0.0.0.0:8004", r)
	}()
	go func() {
		// 接收到信号
